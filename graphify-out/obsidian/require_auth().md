---
source_file: "ultron-discord-bot/bot.py"
type: "code"
community: "Discord Test Fakes"
location: "L561"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Discord_Test_Fakes
---

# require_auth()

## Connections
- [[Returns True if allowed; otherwise sends the Not authorized reply itself and…]] - `rationale_for` [EXTRACTED]
- [[ask_command()]] - `calls` [EXTRACTED]
- [[backup_command()]] - `calls` [EXTRACTED]
- [[bot.py]] - `contains` [EXTRACTED]
- [[containers_command()]] - `calls` [EXTRACTED]
- [[deploy_command()]] - `calls` [EXTRACTED]
- [[diff_command()]] - `calls` [EXTRACTED]
- [[export_command()]] - `calls` [EXTRACTED]
- [[forget_command()]] - `calls` [EXTRACTED]
- [[is_authorized()]] - `calls` [EXTRACTED]
- [[mcp_command()]] - `calls` [EXTRACTED]
- [[portfolio_command()]] - `calls` [EXTRACTED]
- [[repos_command()]] - `calls` [EXTRACTED]
- [[status_command()]] - `calls` [EXTRACTED]
- [[storage_command()]] - `calls` [EXTRACTED]
- [[systems_command()]] - `calls` [EXTRACTED]
- [[trades_command()]] - `calls` [EXTRACTED]
- [[usage_command()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Discord_Test_Fakes