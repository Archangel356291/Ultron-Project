    ...
    try:
        ...
        if conn.execute("SELECT COUNT(*) FROM launch_items").fetchone()[0] == 0:
            ...
            for _k, _n, _st, _note in (
                ("product", "Keeper of Small Fires tee", "Design", "design + listing ready \u2014 next: list on Printful"),
                ("product", "Cozy candle line (wood + vanilla)", "Idea", "top SPAMADR pick; waiting on Printify"),
                ("campaign", "Video 1 \u2014 The keeper", "Scripted", "script ready in FIRST-WEEK-PROMO.md"),
                ("campaign", "Video 2 \u2014 Cozy goblin", "Scripted", "script ready"),
                ("campaign", "Video 3 \u2014 Gift angle", "Scripted", "script ready"),
            ):
                conn.execute("INSERT INTO launch_items (kind, name, stage, note, created_at, updated_at) VALUES (?,?,?,?,?,?)",
                             (_k, _n, _st, _note, _now, _now))
...
AGENT_REGISTRY = {
    ...
    "store_merchant": {
        "role": "Kaupmadr — storefront merchant: drafts listings, titles, SEO copy & descriptions for Shopify/POD",
        "kind": "claude-code", "models": "sonnet",
        "tools": "Read, Grep, Glob; drafts text only (you approve & publish)",
        "forbidden": "publishing or pricing without approval, moving money, payment/payout settings, creating accounts",
        "scope": "the owner's own storefronts, via the owner's own keys",
    },
    "store_reckoner": {
        "role": "Skattmadr — counting house: reconciles orders, tallies sales & tax into the ledger, flags tax owed",
        "kind": "claude-code", "models": "haiku",
        "tools": "the storefront ledger + /api/storefronts (read + record)",
        "forbidden": "filing taxes, moving money, editing orders on the platform",
        "scope": "the storefront sales ledger + records",
    },
    "store_designer": {
        "role": "Design-smith — drafts print-on-demand design CONCEPTS, themes & prompts for the owner to render/approve",
        "kind": "claude-code", "models": "sonnet",
        "tools": "Read, Grep, Glob; concept/text drafts only",
        "forbidden": "claiming finished sellable art, ordering goods, spending money",
        "scope": "design concepts for the owner's POD store",
    },
    "store_sourcer": {
        "role": "Sourcer — researches suppliers for products (e.g. candles) & POD items, compares, drafts a sourcing shortlist",
        "kind": "claude-code", "models": "sonnet",
        "tools": "Read, Grep, Glob, private web research",
        "forbidden": "placing orders, entering payment details, committing spend",
        "scope": "supplier research for the owner's storefronts",
    },
    "store_scout": {
        "role": "Market seer — researches trending/best-selling products so the store stocks what sells; drafts a whats-hot report",
        "kind": "claude-code", "models": "sonnet",
        "tools": "Read, Grep, Glob, private web research",
    ...
    "store_records": {
        "role": "Skrifari — records keeper: logs every product/campaign stage, sale, and supplier/design decision so nothing is lost before launch",
        "kind": "claude-code", "models": "haiku",
        "tools": "the launch board + ledgers (read + append records)",
        "forbidden": "deleting/altering past records, logging secrets or card numbers, acting on the records",
        "scope": "the store's launch board, sales and wallet ledgers",
    },
    "store_support": {
        "role": "Thegn — the host: drafts customer-service replies, order-status answers and an FAQ (incl. the split-shipment note); the owner sends them",
        "kind": "claude-code", "models": "haiku",
        "tools": "Read, Grep, Glob; drafts only",
        "forbidden": "sending messages, issuing refunds, promising what the store cannot do, sharing customer data",
        "scope": "draft replies for the owner's store",
    },
    "store_fulfillment": {
        "role": "Sendimadr — the sender: tracks fulfillment/shipping status from the POD supplier and flags stuck or split orders",
        "kind": "claude-code", "models": "haiku",
        "tools": "Read, Grep, Glob; status reads + flags",
        "forbidden": "placing/cancelling orders, changing addresses, spending, contacting customers directly",
        "scope": "read-only fulfillment status for the owner's store",
    },
    "sentinel": {
...
# ---------------------------------------------------------------------------
# Storefronts -- opt-in print-on-demand sales ledger. Records every sale with
# its tax, keeps a browsable on-disk log folder (per-storefront CSV + a master
# JSONL), and exposes totals + an on/off toggle. The AI can TRACK and DRAFT;
# money movement, payouts and tax filing stay with the human (Module 11).
# Real Etsy/Shopify/Printful sync is wired per-user once they supply their own
# API keys (never the owner's) -- see the multi-tenant config work.
# ---------------------------------------------------------------------------
STOREFRONT_LEDGER_DIR = os.path.join(DATA_DIR, "storefront-ledger")


def _ledger_paths(storefront_id):
    os.makedirs(STOREFRONT_LEDGER_DIR, exist_ok=True)
    return (os.path.join(STOREFRONT_LEDGER_DIR, "ledger.jsonl"),
            os.path.join(STOREFRONT_LEDGER_DIR, "sales-%s.csv" % storefront_id))


def _write_ledger_files(row):
    """Detailed, organized on-disk record for every sale (owner-browsable)."""
    master, per = _ledger_paths(row["storefront_id"])
    try:
        with open(master, "a", encoding="utf-8") as f:
            f.write(json.dumps(row, ensure_ascii=False) + "\n")
        newfile = not os.path.exists(per) or os.path.getsize(per) == 0
        with open(per, "a", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
            if newfile:
                w.writerow(["timestamp", "order_id", "item", "qty", "subtotal", "tax", "total", "currency", "tax_rate", "source"])
            w.writerow([row["ts"], row["order_id"], row["item"], row["qty"],
                        "%.2f" % (row["subtotal_cents"] / 100), "%.2f" % (row["tax_cents"] / 100),
                        "%.2f" % (row["total_cents"] / 100), row["currency"], row["tax_rate"], row["source"]])
    except Exception:
        pass  # a logging failure must never lose the recorded sale (it is in the DB)


def record_storefront_sale(storefront_id, body):
    conn = _get_db_connection()
    try:
        if not conn.execute("SELECT id FROM storefronts WHERE id=?", (storefront_id,)).fetchone():
            return {"error": "unknown storefront"}
        try:
            item = (str(body.get("item", "")).strip()[:200]) or "item"
        ...
        order_id = str(body.get("order_id", ""))[:80] or ("man-" + time.strftime("%Y%m%d%H%M%S"))
...
def storefront_recent(storefront_id, limit=20):
    conn = _get_db_connection()
    try:
        rows = conn.execute("SELECT ts, order_id, item, qty, subtotal_cents, tax_cents, total_cents, currency, tax_rate, source "
                            "FROM storefront_sales WHERE storefront_id=? ORDER BY id DESC LIMIT ?",
                            (storefront_id, int(limit))).fetchall()
        return [{"ts": r["ts"], "order_id": r["order_id"], "item": r["item"], "qty": r["qty"],
                 "subtotal": round(r["subtotal_cents"] / 100, 2), "tax": round(r["tax_cents"] / 100, 2),
                 "total": round(r["total_cents"] / 100, 2), "currency": r["currency"],
                 "tax_rate": r["tax_rate"], "source": r["source"]} for r in rows]
    finally:
        conn.close()


def set_storefront_active(storefront_id, active):
    conn = _get_db_connection()
    try:
        cur = conn.execute("UPDATE storefronts SET active=? WHERE id=?", (1 if active else 0, storefront_id))
        conn.commit()
        if cur.rowcount:
            log_activity("storefront_toggle", "%s -> %s" % (storefront_id, "active" if active else "off"))
...
@require_role
def api_storefronts():
    return jsonify(storefront_summary())


@app.route("/api/storefronts/<storefront_id>/sales", methods=["GET", "POST"])
@require_token
def api_storefront_sales(storefront_id):
    if request.method == "POST":
        result = record_storefront_sale(storefront_id, request.get_json(silent=True) or {})
        return jsonify(result), (201 if result.get("ok") else 400)
    return jsonify({"sales": storefront_recent(storefront_id, request.args.get("limit", 20))})


@app.route("/api/storefronts/<storefront_id>/active", methods=["POST"])
@require_token
...
# ---------------------------------------------------------------------------
# Live Shopify order sync. Uses the CALLER's own SHOPIFY_STORE +
# SHOPIFY_ADMIN_TOKEN from the vault (never the owner's, per get_user_key) to
# pull real orders into the ledger, deduped by order id. Read-only against
# Shopify; it never changes anything on the store or moves money.
# ---------------------------------------------------------------------------
def _shopify_fetch_orders(store, token, limit=100):
    if not store or not token:
        return None, "no Shopify store/token set -- add them under 'Your API keys'"
    store = str(store).strip().replace("https://", "").replace("http://", "").strip("/")
    if not store.endswith("myshopify.com") and "." not in store:
        store = store + ".myshopify.com"
    url = "https://%s/admin/api/2024-01/orders.json?status=any&limit=%d" % (store, max(1, min(250, int(limit))))
    req = urllib.request.Request(url, headers={"X-Shopify-Access-Token": str(token), "Accept": "application/json"})
    try:
        with urllib.request.urlopen(req, timeout=12) as r:
            data = json.load(r)
        return (data.get("orders") or []), None
    except urllib.error.HTTPError as e:
        return None, "Shopify returned HTTP %s (check the store domain, token, and read_orders scope)" % e.code
    except Exception as e:
        return None, ("could not reach Shopify: " + str(e))[:160]


def _shopify_record_orders(storefront_id, orders):
    conn = _get_db_connection()
    try:
        existing = set(row[0] for row in conn.execute(
            "SELECT order_id FROM storefront_sales WHERE storefront_id=?", (storefront_id,)).fetchall())
    finally:
        conn.close()
    synced = skipped = 0
    for o in orders:
        oid = "shop-" + str(o.get("id") or o.get("name") or "")
        if oid == "shop-" or oid in existing:
            skipped += 1
            continue
        try:
            subtotal = float(o.get("current_subtotal_price") or o.get("subtotal_price") or 0)
            tax = float(o.get("current_total_tax") or o.get("total_tax") or 0)
            li = o.get("line_items") or []
            item = ((li[0].get("title") if li else None) or "Shopify order")
            qty = sum(int(x.get("quantity", 1) or 1) for x in li) or 1
        except Exception:
            skipped += 1
            continue
        record_storefront_sale(storefront_id, {"item": str(item)[:200], "qty": qty, "subtotal": subtotal,
                                               "tax": tax, "order_id": oid, "source": "shopify"})
        existing.add(oid)
        synced += 1
    return {"ok": True, "synced": synced, "skipped": skipped}


def shopify_sync(storefront_id):
    store = get_user_key("SHOPIFY_STORE")
    token = get_user_key("SHOPIFY_ADMIN_TOKEN")
    orders, err = _shopify_fetch_orders(store, token)
    if err:
        return {"error": err}
    result = _shopify_record_orders(storefront_id, orders)
    log_activity("storefront_sync", "Shopify sync on %s" % storefront_id,
                 "synced %d, skipped %d" % (result["synced"], result["skipped"]))
    return result
...
def api_storefront_sync(storefront_id):
    ...
    try:
        if not conn.execute("SELECT id FROM storefronts WHERE id=?", (storefront_id,)).fetchone():
            return jsonify({"error": "unknown storefront"}), 400
    finally:
        conn.close()
...
# sum per asset. Every entry is written to a browsable log folder so nothing is
# lost. (Truly live on-chain balances would need the owner's wallet address + an
# explorer API -- a later add; this keeps the honest record in the meantime.)
# ---------------------------------------------------------------------------
WALLET_LEDGER_DIR = os.path.join(DATA_DIR, "wallet-ledger")
WALLET_KINDS = {
    "deposit": ("Deposit", 1),
    "mining_reward": ("Mining reward", 1),
...
def _wallet_ledger_paths(asset):
    os.makedirs(WALLET_LEDGER_DIR, exist_ok=True)
    safe = "".join(c for c in asset if c.isalnum()) or "asset"
    return (os.path.join(WALLET_LEDGER_DIR, "wallet.jsonl"),
            os.path.join(WALLET_LEDGER_DIR, "wallet-%s.csv" % safe))


def _write_wallet_ledger(row):
    master, per = _wallet_ledger_paths(row["asset"])
    try:
        with open(master, "a", encoding="utf-8") as f:
            f.write(json.dumps(row, ensure_ascii=False) + "\n")
        newfile = not os.path.exists(per) or os.path.getsize(per) == 0
        with open(per, "a", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
            if newfile:
                w.writerow(["timestamp", "type", "asset", "amount", "wallet", "note"])
            w.writerow([row["ts"], row["kind_label"], row["asset"], row["amount"], row["wallet_label"], row["note"]])
    except Exception:
        pass  # never lose the DB row over a logging failure


def record_wallet_tx(body):
    kind = str(body.get("kind", "")).strip()
    if kind not in WALLET_KINDS:
        return {"error": "unknown transaction type"}
    asset = (str(body.get("asset", "BTC")).strip().upper()[:12]) or "BTC"
    try:
        amt = float(body.get("amount", 0))
    except (TypeError, ValueError):
        return {"error": "bad amount"}
    label_name, sign = WALLET_KINDS[kind]
    signed = amt if sign == 0 else abs(amt) * sign
    if kind != "adjustment" and amt == 0:
        return {"error": "amount required"}
    wallet_label = str(body.get("wallet_label", "")).strip()[:60]
    note = str(body.get("note", "")).strip()[:280]
    ts = time.strftime("%Y-%m-%dT%H:%M:%S")
    conn = _get_db_connection()
    try:
        conn.execute("INSERT INTO wallet_transactions (ts, kind, asset, amount, wallet_label, note) VALUES (?,?,?,?,?,?)",
                     (ts, kind, asset, signed, wallet_label, note))
        conn.commit()
    finally:
        conn.close()
    row = {"ts": ts, "kind": kind, "kind_label": label_name, "asset": asset,
           "amount": round(signed, 10), "wallet_label": wallet_label, "note": note}
    _write_wallet_ledger(row)
    log_activity("wallet_tx", "%s %s %s" % (label_name, round(signed, 10), asset), (wallet_label + " " + note).strip()[:200])
...
def wallet_summary():
    ...
    try:
        ...
        for r in conn.execute(
            "SELECT asset, COUNT(*) c, COALESCE(SUM(amount),0) bal, "
            "COALESCE(SUM(CASE WHEN amount>0 THEN amount ELSE 0 END),0) tin, "
            "COALESCE(SUM(CASE WHEN amount<0 THEN -amount ELSE 0 END),0) tout "
            "FROM wallet_transactions GROUP BY asset ORDER BY asset").fetchall():
            assets.append({"asset": r["asset"], "balance": round(r["bal"], 10),
                           "added": round(r["tin"], 10), "removed": round(r["tout"], 10), "count": r["c"]})
        recent = [{"ts": x["ts"], "kind": x["kind"], "kind_label": WALLET_KINDS.get(x["kind"], (x["kind"], 0))[0],
                   "asset": x["asset"], "amount": round(x["amount"], 10), "wallet_label": x["wallet_label"], "note": x["note"]}
                  for x in conn.execute("SELECT ts, kind, asset, amount, wallet_label, note FROM wallet_transactions ORDER BY id DESC LIMIT 40").fetchall()]
        return {"assets": assets, "recent": recent, "kinds": [{"key": k, "label": v[0]} for k, v in WALLET_KINDS.items()],
                "ledger_dir": WALLET_LEDGER_DIR}
    finally:
        conn.close()
...
@app.route("/api/wallet/tx", methods=["POST"])
@require_token
def api_wallet_tx():
    result = record_wallet_tx(request.get_json(silent=True) or {})
    return jsonify(result), (201 if result.get("ok") else 400)


# ---------------------------------------------------------------------------
# Launch board -- track products & ad campaigns from idea to live, so nothing is
# lost before launch. Records every stage change to a log folder (the records
# keeper). Not autonomous; the owner moves items along.
# ---------------------------------------------------------------------------
LAUNCH_LOG_DIR = os.path.join(DATA_DIR, "launch-log")
LAUNCH_LADDERS = {
    "product": ["Idea", "Design", "Sample", "Listed", "Live"],
    "campaign": ["Idea", "Scripted", "Filmed", "Scheduled", "Posted"],
}


def _launch_log(entry):
    try:
        os.makedirs(LAUNCH_LOG_DIR, exist_ok=True)
        with open(os.path.join(LAUNCH_LOG_DIR, "launch.jsonl"), "a", encoding="utf-8") as f:
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")
    except Exception:
        pass


def launch_upsert(body):
    note = str(body.get("note", "")).strip()[:280]
    now = time.strftime("%Y-%m-%dT%H:%M:%S")
    conn = _get_db_connection()
    try:
        item_id = body.get("id")
        if item_id:
            row = conn.execute("SELECT kind, name FROM launch_items WHERE id=?", (item_id,)).fetchone()
            if not row:
                return {"error": "unknown item"}
            kind = row["kind"]
        else:
            kind = str(body.get("kind", "")).strip()
            if kind not in LAUNCH_LADDERS:
                return {"error": "kind must be product or campaign"}
        ladder = LAUNCH_LADDERS[kind]
        stage = str(body.get("stage", "")).strip()
        if stage and stage not in ladder:
        ...
        if item_id:
            ...
            vals.append(item_id)
            conn.execute("UPDATE launch_items SET " + ", ".join(sets) + " WHERE id=?", vals)
            conn.commit()
            _launch_log({"ts": now, "id": item_id, "name": row["name"], "kind": kind, "stage": stage, "note": note})
            return {"ok": True, "id": item_id}
        name = str(body.get("name", "")).strip()[:120]
        if not name:
            return {"error": "name required"}
        st = stage or ladder[0]
        cur = conn.execute("INSERT INTO launch_items (kind, name, stage, note, created_at, updated_at) VALUES (?,?,?,?,?,?)",
                           (kind, name, st, note, now, now))
        conn.commit()
        _launch_log({"ts": now, "id": cur.lastrowid, "name": name, "kind": kind, "stage": st, "note": note, "created": True})
...
@require_role
def api_launch():
    return jsonify(launch_summary())


@app.route("/api/launch/item", methods=["POST"])
@require_token
def api_launch_item():
